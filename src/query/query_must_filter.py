...
def satisfies_query_must(
    query: str,
    query_must: Optional[Dict[str, Any]],
    debug: bool = False,
    output: Optional["RAGOutput"] = None,
) -> bool:
    """
    ...
    Args:
        ...
        output: Optional RAGOutput buffer for diagnostic messages (uses print if None)
    ...
    if not validate_contain_one_of(query, query_must):
        if debug:
            msg = f"    ❌ Query does not contain_one_of: {query_must.get('contain_one_of')}"
            if output:
                output.info(msg)
            else:
                print(msg)
    ...
    if not validate_contain_all_of(query, query_must):
        if debug:
            msg = f"    ❌ Query does not contain_all_of: {query_must.get('contain_all_of')}"
            if output:
                output.info(msg)
            else:
                print(msg)
    ...
    if not validate_contain(query, query_must):
        if debug:
            msg = f"    ❌ Query does not contain: {query_must.get('contain')}"
            if output:
                output.info(msg)
            else:
                print(msg)
    ...
    if not validate_contain_range(query, query_must):
        if debug:
            msg = f"    ❌ Query does not contain_range: {query_must.get('contain_range')}"
            if output:
                output.info(msg)
            else:
                print(msg)
...
if __name__ == "__main__":
    ...
    query_must1 = {
        "contain_one_of": [["cleric", "clerics"], ["armor class 6", "ac 6", "a.c. 6"]]
    }
    ...
    query_must3 = {
        "contain_one_of": [["psionic", "psionics"], ["intelligence", "int"]],
