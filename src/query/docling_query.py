...
class DnDRAG:
    def __init__(
        ...
        self.output.info(
            f"  ChromaDB: {self.chroma.chroma_host}:{self.chroma.chroma_port}"
        )
        ...
        try:
            ...
            self.output.error(
                f"     Available collections: {[c.name for c in self.chroma.list_collections()]}"
            )
            raise
    ...
    def _format_chunk_info(
        self, metadata: dict, current_num: int = None, total_num: int = None
    ) -> str:
    ...
    def retrieve(
        ...
        if enable_filtering:
            ...
            return self._retrieve_with_filtering(
                query, k, distance_threshold, debug, max_iterations
            )
        else:
    ...
    def _retrieve_base(
        ...
        for pattern in comparison_patterns:
            ...
            if match:
                ...
                for stop_word in ["summarize", "what are", "how do", "explain"]:
                    entity2 = re.sub(
                        rf"\s+{stop_word}.*$", "", entity2, flags=re.IGNORECASE
                    )
        ...
        results = self.collection.query(
            query_embeddings=[query_embedding], n_results=expanded_k
        )

        # If entities were detected, ensure they're all in the results
        if entities_mentioned:
            retrieved_names = [
                meta.get("name", "").lower() for meta in results["metadatas"][0]
            ]
            missing_entities = []
            ...
            if missing_entities:
                for entity in missing_entities:
                    entity_results = self.collection.query(
                        query_embeddings=[self.get_embedding(entity)], n_results=1
                    )
                    if entity_results["ids"][0]:
                        # Add this result to our main results
                        results["ids"][0].append(entity_results["ids"][0][0])
                        results["documents"][0].append(
                            entity_results["documents"][0][0]
                        )
                        results["metadatas"][0].append(
                            entity_results["metadatas"][0][0]
                        )
                        results["distances"][0].append(
                            entity_results["distances"][0][0]
                        )
        ...
        if results["metadatas"][0] and results["metadatas"][0][0].get(
            "parent_category_id"
        ):
        ...
        if len(results["ids"][0]) > 0:
            ...
            if debug:
                self.output.info(
                    f"\n[GAP ANALYSIS] Adaptive cutoff to remove low-relevance results"
                )
                self.output.info(
                    f"  Strategy: Look for 'semantic cliffs' - large gaps in similarity scores"
                )
                self.output.info(
                    f"  If no cliff found, use distance threshold (best + {distance_threshold})"
                )
                self.output.info(
                    f"\n  Retrieved {len(distances)} chunks with distances:"
                )
                for i, (dist, meta) in enumerate(
                    zip(distances, results["metadatas"][0])
                ):
                    name = meta.get("name", meta.get("title", "Unknown"))
                    self.output.info(f"    {i+1}. {name[:50]:<50} distance={dist:.4f}")

            # Calculate gaps between consecutive results
            # Start from index 2 (skip gap after chunk 1, which can be large
            # simply because chunk 1 is exceptionally good)
            gaps = []
            for i in range(2, min(len(distances), k)):
                gap = distances[i] - distances[i - 1]
                gaps.append((i, gap))  # (0-indexed position after gap, gap_size)
            ...
            if debug and gaps:
                self.output.info(
                    f"\n  Analyzing gaps between consecutive chunks (skipping gap after chunk 1):"
                )
                for pos, gap in sorted(gaps, key=lambda x: x[1], reverse=True)[:5]:
                    self.output.info(
                        f"    Gap between chunks {pos} and {pos+1}: {gap:.4f}"
                    )
                self.output.info(
                    f"\n  Largest gap: {max_gap_size:.4f} (between chunks {max_gap_pos} and {max_gap_pos+1})"
                )
                self.output.info(
                    f"  Gap threshold for 'semantic cliff': {gap_threshold}"
                )
                if max_gap_size >= gap_threshold:
                    self.output.info(
                        f"  ✓ Cliff detected! Gap {max_gap_size:.4f} >= {gap_threshold}"
                    )
                else:
                    self.output.info(
                        f"  ✗ No cliff detected (largest gap {max_gap_size:.4f} < {gap_threshold})"
                    )
            ...
            if max_gap_size >= gap_threshold:
                ...
                strategy_used = f"gap detection (cliff between chunks {max_gap_pos} and {max_gap_pos+1}, gap={max_gap_size:.4f})"
            ...
            keep_count = (
                max(2, keep_count) if len(distances) > 1 else 1
            )  # Min 2 results
            ...
            if debug:
                ...
                if original_keep != keep_count:
                    self.output.info(
                        f"  Applying constraints: {original_keep} → {keep_count} chunks"
                    )
                    if keep_count == 2 and len(distances) > 1:
                        self.output.info(f"    - Enforcing minimum of 2 results")
                self.output.info(
                    f"\n  Final result: Keeping top {keep_count} of {len(distances)} chunks"
                )
                if keep_count < len(distances):
                    self.output.info(
                        f"  Dropping {len(distances) - keep_count} low-relevance chunks:"
                    )
                    for i, (dist, meta) in enumerate(
                        zip(
                            distances[keep_count:], results["metadatas"][0][keep_count:]
                        ),
                        keep_count + 1,
                    ):
                        name = meta.get("name", meta.get("title", "Unknown"))
                        self.output.info(
                            f"    {i}. {name[:50]:<50} distance={dist:.4f}"
                        )
    ...
    def _retrieve_with_filtering(
        ...
        while iteration < max_iterations:
            ...
            if all_seen_ids:
                ...
                if debug:
                    self.output.info(
                        f"[FILTERING] Excluding {len(all_seen_ids)} previously processed chunks ({len(kept_ids)} kept + {len(excluded_ids)} excluded)"
                    )
            ...
            if not results["ids"][0]:
                if debug:
                    self.output.info(
                        f"[FILTERING] No more results available from ChromaDB"
                    )
                break
            ...
            for chunk_id, metadata, document, distance in zip(
                ...
                if uid in kept_ids:
                    if debug:
                        chunk_info = self._format_chunk_info(
                            metadata, chunk_counter, total_chunks_in_batch
                        )
                ...
                is_reference = (
                    ...
                    and metadata.get("section") == "EXPLANATORY NOTES"
                )
                ""
                ...
                if "query_must" in metadata and not is_reference:
                    try:
                        ...
                        query_must = (
                            json.loads(metadata["query_must"])
                            if isinstance(metadata["query_must"], str)
                            else metadata["query_must"]
                        )

                        # Check if query satisfies requirements
                        if satisfies_query_must(
                            query, query_must, debug=debug, output=self.output
                        ):
                            newly_kept.append(
                                {
                                    "id": chunk_id,
                        ...
                        if debug:
                            ...
                            self.output.info(
                                f"  ⚠️  KEEP (malformed query_must): {chunk_info}"
                            )
                        newly_kept.append(
                            {
                                "id": chunk_id,
                    ...
                    if debug:
                        ...
                        if is_reference:
                            self.output.info(
                                f"  ✅ KEEP: {chunk_info} (reference - no filtering)"
                            )
                        else:
                            self.output.info(
                                f"  ✅ KEEP: {chunk_info} (no restrictions)"
                            )
            ...
            if debug:
                self.output.info(
                    f"[FILTERING] Kept {len(newly_kept)}, excluded {len(newly_excluded)} (total kept: {len(all_kept_chunks)})"
                )
            ...
            if len(all_kept_chunks) >= k:
                if debug:
                    self.output.info(
                        f"[FILTERING] Target k={k} reached ({len(all_kept_chunks)} chunks)"
                    )
                break
            ...
            if len(newly_excluded) == 0 and len(newly_kept) == 0:
                if debug:
                    self.output.info(
                        f"[FILTERING] No new chunks this iteration (all were duplicates), stopping"
                    )
                break
            ...
            if len(newly_excluded) == 0:
                if debug:
                    self.output.info(
                        f"[FILTERING] No exclusions this iteration, stopping"
                    )
                break
        ...
        if len(filtered_results["ids"][0]) > 0:
            ...
            if debug:
                self.output.info(f"\n[GAP ANALYSIS] Post-filtering adaptive cutoff")
                self.output.info(f"  Analyzing {len(distances)} filtered chunks:")
                for i, (dist, meta) in enumerate(
                    zip(distances, filtered_results["metadatas"][0])
                ):
                    name = meta.get("name", meta.get("title", "Unknown"))
                    self.output.info(f"    {i+1}. {name[:50]:<50} distance={dist:.4f}")
            ...
            if debug and gaps:
                self.output.info(
                    f"\n  Gaps between consecutive chunks (skipping gap after chunk 1):"
                )
                for pos, gap in sorted(gaps, key=lambda x: x[1], reverse=True)[:5]:
                    self.output.info(
                        f"    Gap between chunks {pos} and {pos+1}: {gap:.4f}"
                ...
                if max_gap_size >= gap_threshold:
                    self.output.info(
                        f"  ✓ Cliff detected! Keeping top {max_gap_pos} chunks"
                    )
                else:
                    self.output.info(f"  ✗ No cliff detected, using distance threshold")
            ...
            if debug:
                ...
                self.output.info(
                    f"  Final result: Returning {keep_count} chunks after filtering and gap analysis"
                )

            # Trim results
            if keep_count < len(filtered_results["ids"][0]):
                filtered_results["ids"][0] = filtered_results["ids"][0][:keep_count]
                filtered_results["documents"][0] = filtered_results["documents"][0][
                    :keep_count
                ]
                filtered_results["metadatas"][0] = filtered_results["metadatas"][0][
                    :keep_count
                ]
                filtered_results["distances"][0] = filtered_results["distances"][0][
                    :keep_count
                ]
...
Answer based on the context above:"""
    ...
    def query(
        ...
        self.output.info(
            f"Retrieving up to {k} relevant chunks ({filter_status}, distance threshold: {distance_threshold})..."
        )
...
def main():
    parser = argparse.ArgumentParser(description="Query D&D 1st Edition RAG system")
    parser.add_argument(
        "query",
        nargs="?",
    ...
    parser.add_argument(
        "--show-context", action="store_true", help="Show the context sent to the LLM"
    )
    parser.add_argument(
        "--debug",
        action="store_true",
    ...
    parser.add_argument(
        "--test", action="store_true", help="Run test questions for the collection"
    )
